...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [block, below of=store1, yshift=-1cm] (mine) {Mine source(s)};
    \node [decision, below of=store2, yshift=-0.5cm] (decide) {Start to Mine?};
    \node [block, below of=mine, yshift=-1cm] (store3) {Store and share it};
%     \node [block, below of=decide, node distance=3cm] (stop) {stop};
    % Draw edges
     \path [line] (init) -- (explore);
     \path [line] (explore) -- node [anchor=east] {New source found}(store1);
     \path [line] (explore) -- node {New source received}(store2);
     \path [line] (store1) -- (mine);
     \path [line] (store2) -- (decide);
     \path [line] (decide) -- ++(2cm,0) node[anchor=north] {no} --++(0,5cm) -| (explore);
     \path [line] (decide) -- ++(0,-2cm) node [anchor=west]{yes} |-(mine);
     \path [line] (mine) -- node {New source found or received}(store3);
     \path [line] (store3) -- ++(-2cm,0) |- (mine);
