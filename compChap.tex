...
\begin{figure}[htb]
  \begin{tikzpicture}[node distance = 2cm,>=stealth, auto]
    ...
    % Draw edges
     ...
     \path [line] (decide) -- ++(2cm,0) node[xshift=.5cm, yshift=.7cm] {No} --++(0,5cm) -| (explore);
...
\end{tikzpicture}
\caption{Flowchart décrivant le comportement haut niveau de chaque robot~\cite{foraging}.\label{fig:genFlowchart}}
\end{figure}
